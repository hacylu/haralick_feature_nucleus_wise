function [GLCMS,SI] = graycomatrix_myversion(varargin)
...
% the code is further edited by Haojia Li to take account of the ROI in the
% input image so that the counting of gray level pair occurence only
% happens in ROI


%   Copyright 1993-2012 The MathWorks, Inc.

[I,roi_mask Offset, NL, GL, makeSymmetric] = ParseInputs(varargin{:});
if ~islogical(roi_mask)
    error('ROI mask must be logical')
end
...
if NL ~= 0
  ...
  for k = 1 : numOffsets
    GLCMS(:,:,k) = computeGLCM(r,c,roi_mask,Offset(k,:),SI,NL);
...
%-----------------------------------------------------------------------------
function oneGLCM = computeGLCM(r,c,roi_mask,offset,si,nl)
...
v1 = shiftdim(si,1);
roi_mask = shiftdim(roi_mask,1);

v1 = v1(:);
roi_label=roi_mask(:);

v1(outsideBounds) = [];
roi_label(outsideBounds) = [];
...
if isempty(Ind)
    ...
    oneGLCM = accumarray(Ind(find(roi_label),:), 1, [nl nl]);
end

%-----------------------------------------------------------------------------
function [I,roi_mask,offset, nl, gl, sym] = ParseInputs(varargin)
